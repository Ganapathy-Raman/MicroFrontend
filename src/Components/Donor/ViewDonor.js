...
function ViewDonor() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:7878/donor/all")
      .then((response) => {
  ...
  const handleSubmit = (donorId) => {
    const conf = window.confirm("Do you want to delete this record?");
    if (conf) {
      axios
        .delete("http://localhost:7878/donor/" + donorId)
        .then(() => {
          alert("Donor record has been deleted");
          setRecords(records.filter(record => record.donorId !== donorId));
        })
    ...
    <>
      ...
      <div className="container mx-auto p-6">
        <div className="relative mb-8">
          <input
            type="text"
            placeholder="Search for available donors..."
            value={searchQuery}
                ...
                <tr key={i} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 flex space-x-2">
                    ...
                    <button
                      ...
                      title="Delete"
                      onClick={() => handleSubmit(d.donorId)}
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        className="w-5 h-5"
