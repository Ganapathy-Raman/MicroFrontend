...
function ViewRecipient() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:7878/recipient/all")
      .then((response) => {
  ...
  const handleSubmit = (recipientId) => {
    const conf = window.confirm("Do you want to delete this record?");
    if (conf) {
      axios
        .delete("http://localhost:7878/recipient/" + recipientId)
        .then(() => {
          alert("Recipient record has been deleted");
          setRecords(records.filter(record => record.recipientId !== recipientId));
        })
    ...
    <>
      ...
      <div className="container mx-auto p-6">
        <div className="relative mb-8">
          <input
            type="text"
            placeholder="Search for available recipients..."
            value={searchQuery}
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredRecords.map((d, i) => (
                <tr key={i} className="hover:bg-gray-50">
                  <td className="px-4 py-3 text-gray-800">{d.recipientId}</td>
                  <td className="px-4 py-3 text-gray-800">{d.userId.userId  }</td>
                  <td className="px-4 py-3 text-gray-800">{d.recipientName}</td>
                  <td className="px-4 py-3 text-gray-800">{d.contactNo}</td>
                  <td className="px-4 py-3 text-gray-800">{d.dateOfAvailabity}</td>
                  <td className="px-4 py-3 text-gray-800">{d.organNeeded}</td>
                  <td className="px-4 py-3 text-gray-800">{d.address}</td>
                  <td className="px-4 py-3 text-gray-800">
                    <button onClick={() => viewMd(d.medicalCondition)}>
                      <FontAwesomeIcon icon={getFileIcon(d.medicalCondition)} size="lg" />
                    </button></td>
                  <td className="px-4 py-3 text-gray-800">{d.status}</td>
                  <td className="px-4 py-3 flex space-x-2">
                    <Link to={`/editRecipient/${d.recipientId}`}>
                      <button
                    ...
                    <button
                      ...
                      title="Delete"
                      onClick={() => handleSubmit(d.recipientId)}
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        className="w-5 h-5"
