...
function ViewApplication() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:7878/application/all")
      .then((response) => {
  ...
  const handleSubmit = (applicationId) => {
    ...
    if (conf) {
      axios
        .delete("http://localhost:7878/application/" + applicationId)
        .then(() => {
          alert("Application record has been deleted");
          setRecords(records.filter(record => record.applicationId !== applicationId));
        })
    ...
    <>
      ...
      <div className="container mx-auto p-6">
        <div className="relative mb-8">
          <input
            type="text"
            placeholder="Search for available applications..."
            value={searchQuery}
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredRecords.map((d, i) => (
                <tr key={i} className="hover:bg-gray-50">
                  <td className="px-4 py-3 text-gray-800">{d.applicationId}</td>
                  <td className="px-4 py-3 text-gray-800">{d.recipientId.recipientId}</td>
                  <td className="px-4 py-3 text-gray-800">{d.bloodGroup}</td>
                  <td className="px-4 py-3 text-gray-800">{d.age}</td>
                  <td className="px-4 py-3 text-gray-800">{d.details}</td>
                  <td className="px-4 py-3 text-gray-800">{d.status}</td>
                  <td className="px-4 py-3 flex space-x-2">
                    <Link to={`/editApplication/${d.applicationId}`}>
                      <button
