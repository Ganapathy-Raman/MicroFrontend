...
function ViewMedicalRecord() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:7878/medicalRecord/all")
      .then((response) => {
  ...
  const handleSubmit = (recordId) => {
    ...
    if (conf) {
      axios
        .delete("http://localhost:7878/medicalRecord/" + recordId)
        .then(() => {
    ...
    <>
      ...
      <div className="container mx-auto p-6">
        <div className="relative mb-8">
          <input
            type="text"
            placeholder="Search for available medical records..."
            value={searchQuery}
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredRecords.map((d, i) => (
                <tr key={i} className="hover:bg-gray-50">
                  <td className="px-4 py-3 text-gray-800">{d.recordId}</td>
                  ...
                  <td className="px-4 py-3 flex space-x-2">
                    <Link to={`/editmedicalRecord/${d.recordId}`}>
                      <button
                    ...
                    <button
                      ...
                      title="Delete"
                      onClick={() => handleSubmit(d.recordId)}
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        className="w-5 h-5"
